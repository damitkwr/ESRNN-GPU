...
class DRNN(nn.Module):
    ...
    def base_layer(self, inp, lstm_cell, rate, hidden):
        steps = len(inp)
        b_size = inp[0].size()[0]
        ...
        if hidden is None:
            ...
            if self.out_type == 'stack':
                hidden = torch.stack([hidden[i::rate, :, :] for i in range(rate)], 2)
            else:
    ...
    def pad_inp(self, inp, steps, rate):
        ...
        if steps % rate != 0:
            ...
            new_inp = torch.cat((inp, torch.autograd.Variable(pad)))  # have to make it Autograd variable otherwise PyTorch might not calculate gradients
        else:
